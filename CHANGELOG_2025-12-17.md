...
- **日期**: 2025-12-17
- **版本**: v3.0.3
- **主要改进**: Refresh Token 批量导入、Claude CLI 修复、API 代理优化
...
### 4. Refresh Token 批量导入 (v3.0.3) ✨

**新增功能**: 大幅提升账号添加效率,支持一次性导入多个 Refresh Token。

**支持格式**:
1. **JSON 数组**: 直接粘贴包含 `refresh_token` 字段的 JSON 数组 (例如从数据库导出的数据)
2. **智能提取**: 支持从任意文本中自动识别正则模式 `1//...` 并提取所有 Token
3. **单个 Token**: 兼容原有的单个粘贴方式

**交互体验**:
- 实时显示导入进度 (e.g. "正在导入 2/10...")
- 导入完成后显示成功/失败统计
- 自动去重处理

**截图**:
![批量导入界面](https://github.com/user-attachments/assets/xxxx) *(示意图)*
